...
def get_npz_filename(data_dir, image_key, is_full_path=False, recursive=False):
    if is_full_path:
...
def main(args):
    ...
    if args.min_bucket_reso == args.max_bucket_reso and len(glob.glob(os.path.join(args.output_dir, "*.npz"))) > 0 and args.skip_existing:
